...
export default function AdminDashboard({ user }: AdminDashboardProps) {
  ...
  // Live Activity Feed computation
  const activityLogs = [
    // 1. Application timeline events
    ...applications.flatMap(app => {
      const startupName = app.data?.startupName || app.data?.startupTitle || 'Unknown Startup';
      return (app.timeline || []).map(event => ({
        id: `${app.id}-${event.timestamp}-${event.status}`,
        type: 'application',
        title: event.status,
    ...
    ...meetings.map(m => {
      const relatedApp = applications.find(a => a.id === m.applicationId);
      const startupName = relatedApp?.data?.startupName || relatedApp?.data?.startupTitle || 'System';
      return {
        id: m.id,
        type: 'meeting',
        title: `Meeting Scheduled: ${m.title}`,
        timestamp: m.startTime || Date.now(),
        remarks: `Location/Mode: ${m.location || m.mode || 'Online'}. ${m.description || ''}`,
        startupName,
    ...
    ...events.map(e => ({
      id: e.id,
      type: 'event',
      title: `Event Announced: ${e.title}`,
      timestamp: e.createdAt || Date.now(),
      remarks: `Date: ${e.date} at ${e.time}. Location/Link: ${e.linkOrLocation}`,
      startupName: 'PIERC Incubator',
      link: `/dashboard/events`
    }))
  ].sort((a, b) => b.timestamp - a.timestamp); // Sorted chronologically (newest first)
  ...
  return (
    <div className="space-y-8 animate-page-entry">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-black tracking-tight text-slate-900">Incubation Center Console</h1>
          <p className="text-slate-500 font-medium">PIERC ERP Dashboard Overview for Administrators</p>
        </div>
      ...
      <Tabs defaultValue="overview" className="w-full">
        ...
        {/* Tab 1: Overview Tab containing recommended layout charts */}
        ...
        {/* Tab 2: Detailed Applications Analytics */}
